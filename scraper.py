...
def scraper(ticker, latest_xhr_id='https://api.stocktwits.com/api/2/streams/symbol/TSLA.json/', max_volume=50000, start_date='2021-01-01', end_date='2021-07-04'):
    ...
    for _ in range(max_volume):
        try:
            ...
            response = requests.get(f'https://api.stocktwits.com/api/2/streams/symbol/TSLA.json?max='+scroll_list[-1])
            # print(scroll_list)
...
if __name__ == "__main__":
    tweets_df = scraper('TSLA', max_volume=1500)
    print(tweets_df.head())
    tweets_df.to_csv('tslatweets.csv')